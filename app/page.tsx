...
import { Clock, CheckCircle2, AlertCircle, Package, Users, Briefcase, ChevronRight, ArrowUpRight, TrendingUp } from "lucide-react";
...
export default function Dashboard() {
  ...
  const todaysTasks = [
    ...jobs.filter(j => j.status !== "Job Completed").slice(0, 2).map(j => ({
      id: j.id, type: "Job", title: `${j.type} - ${j.location}`, subtitle: j.customer, status: j.status, time: "10:00 AM", color: "text-blue-500 bg-blue-500/10"
    })),
    ...customers.filter(c => c.status === "Lead").slice(0, 1).map(c => ({
      id: c.id, type: "Measurement", title: c.enquiry, subtitle: c.name, status: "Scheduled", time: "02:30 PM", color: "text-indigo-500 bg-indigo-500/10"
    }))
  ];
              ...
              <div className="divide-y divide-border/50">
                {todaysTasks.map((task, idx) => (
                  <motion.div 
                    ...
                    className="p-6 flex items-start gap-6 transition-colors group cursor-pointer"
                  >
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          <span className={`text-[10px] uppercase tracking-[0.1em] font-black px-2.5 py-1 rounded-lg ${task.color}`}>
                            {task.type}
                          </span>
                        </div>
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <motion.button 
                          whileHover={{ scale: 1.1, backgroundColor: "rgb(16 185 129 / 0.1)" }}
                          whileTap={{ scale: 0.9 }}
                          className="p-2.5 rounded-xl text-muted-foreground hover:text-emerald-500 transition-colors border border-transparent hover:border-emerald-500/20"
                        >
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <Link 
                  key={a.label} 
                  href={a.href} 
                  className={`flex flex-col items-center justify-center gap-3 rounded-2xl p-5 text-xs font-black uppercase tracking-widest transition-all border bg-gradient-to-br ${a.color} hover:-translate-y-1 hover:shadow-lg`}
                >
              ...
              {orders.filter(o => o.status === "Pending Order" || o.status === "Order Placed").slice(0, 3).map(o => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-foreground truncate">{o.item}</p>
                    <p className="text-xs text-muted-foreground font-medium truncate mt-0.5">{o.supplier}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-sm font-black text-foreground">{o.expected}</p>
