...
export default function StocksPage() {
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-800 pb-4 mb-4 gap-4">
      ...
      <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg dark:ring-white/10">
        <table className="min-w-full divide-y divide-slate-300 dark:divide-slate-800">
          <thead className="bg-slate-50 dark:bg-slate-900/50">
            <tr>
              <th scope="col" className="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-slate-900 dark:text-slate-100 sm:pl-6">Item</th>
              <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-slate-900 dark:text-slate-100">Category</th>
              <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-slate-900 dark:text-slate-100">Stock Level</th>
              <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-slate-900 dark:text-slate-100">Status</th>
            </tr>
          </thead>
            ...
            {mockStocks.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                  ...
                  <div className="text-slate-500 dark:text-slate-400 font-mono text-xs mt-1">{item.sku}</div>
                </td>
                <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-500 dark:text-slate-400">
                  {item.category}
                </td>
                <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-700 dark:text-slate-300 font-medium">
                  {item.qty} <span className="text-slate-400 dark:text-slate-500">{item.unit}</span>
                </td>
